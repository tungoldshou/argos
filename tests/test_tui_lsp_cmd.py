...
def test_lsp_reload_invalid_keeps_old(isolated_lsp_home, monkeypatch):
    ...
    from argos_agent.lsp import _reset_config, get_config, reload_config, LspConfigError
    from argos_agent.lsp import config as _lsp_config
...
def test_lsp_cmd_lists_servers(isolated_lsp_home, monkeypatch):
    ...
    from argos_agent.lsp import _reset_config, reload_config
    from argos_agent.lsp import config as _lsp_config
    isolated_lsp_home.mkdir(parents=True, exist_ok=True)
    p = isolated_lsp_home / "lsp.json"
    p.write_text(json.dumps({
    ...
    from argos_agent.lsp import get_manager
    mgr = get_manager()
