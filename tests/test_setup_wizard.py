...
def test_write_profile_splits_secret_and_settings(tmp_path):
    ...
    assert "api_key" not in prof and "secret123" not in json.dumps(cfg)
    env = (tmp_path / ".env").read_text()
    assert "MM_KEY=secret123" in env
...
def test_write_profile_appends_to_existing_config(tmp_path):
    ...
    cfg = json.loads((tmp_path / "config.json").read_text())
    assert set(cfg["models"]) == {"a", "b"} and cfg["active"] == "a"
...
async def test_run_wizard_custom_preset(tmp_path, monkeypatch):
    ...
    assert prof["embedding_model"] == "nomic-embed-text"
...
@pytest.mark.asyncio
async def test_run_wizard_paste_key_warning_uses_config_dir(tmp_path, monkeypatch):
    import argos.setup_wizard as W
    ...
    inputs = iter(["3", "", "paste", "secret123", "n", "", "n"])
    out: list[str] = []

    await W.run(
        reader=lambda p="": next(inputs),
        writer=out.append,
        config_dir=config_dir,
    )

    text = "\n".join(out)
    assert str(config_dir / ".env") in text
    assert "~/.argos" not in text


@pytest.mark.asyncio
async def test_run_wizard_no_tty_help_uses_config_dir(tmp_path):
    import argos.setup_wizard as W
    ...
    await W.run(reader=closed_stdin, writer=out.append, config_dir=config_dir)

    text = "\n".join(out)
    assert str(config_dir / "config.json") in text
    assert str(config_dir / ".env") in text
    assert "~/.argos" not in text
...
def test_ask_int_fail_soft_on_non_numeric():
    ...
    assert _ask_int(lambda p="": "8192", out.append, "max:", 4096) == 8192
    assert _ask_int(lambda p="": "", out.append, "max:", 4096) == 4096
