...
import { reduceEvent, startTurn, type Turn, type Block } from './chatReducer';
...
describe('chatReducer', () => {
  ...
  it('tool_result 补全最近一个无 result 的 activity', () => {
    ...
    const act = t[0].blocks.find((b: Block) => b.kind === 'activity');
  ...
  it('verify_failed / escalation 生成 honesty block', () => {
    ...
    const honesty = t[0].blocks.filter((b: Block) => b.kind === 'honesty');
