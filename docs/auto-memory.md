...
**secret 安全**:写入前先经 `_redact_secrets()`(`sk-ant-*` / `sk-*` / `Bearer *` / `password=*` / 私钥 `BEGIN ... PRIVATE KEY` 等 9 条 regex),redact 后为空则不写。
...
4 层记忆的召回是纯内存打分排序(Append-only JSONL,无 sqlite):`_rank()` 先按 type 优先级、再按 `_score()` 降序取 top N。`_score = recency × confidence`,其中 `recency = exp(-0.01 × days_since_last_used)`。低于最低置信度的条目直接排除,无向量检索。

(注:`argos/memory/store.py` 的 run-memory 是另一子系统,走 sqlite-vec 向量召回 + embedding 不可用时降级 LIKE 字面匹配,与此处的 4 层记忆召回无关。)
