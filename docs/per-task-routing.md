...
> **内核级配置**:routing 配置由 daemon 内核(argosd)读取,对所有接入客户端生效 —— 包括终端 TUI、以及进行中的桌面客户端(desktop/)。修改后重启 daemon 即生效(`pkill argosd`;TUI 启动时自动重新拉起)。
...
> `/routing` 是 TUI 专属命令;桌面客户端(desktop/ — 进行中,尚未发布)将有自己的设置界面。
...
5. **不**动 `ModelClient` 既有方法 / `LoopConfig` 字段 / `core/loop.py` 流程 —— 全部通过
   `router` kw-only 注入 + 循环顶部 `if router` 短路(全部既有测试 0 破坏(router=None 短路路径))
