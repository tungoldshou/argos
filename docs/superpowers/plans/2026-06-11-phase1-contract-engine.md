...
**Architecture:** 新建 `argos/contracts/` 包(types/runner/synthesize 三个文件,纯 Python 无新框架)。exit_code 检查通过注入的 `run_cmd` 回调委托给现有 `Verifier._run_verify`(白名单 + verify_dir 隔离原样保留);产物类检查纯本地文件断言。聚合判决复用 `core.types.Verdict` 三态,fail-closed:无检查/检查本身出错 → unverifiable,绝不蒙混成 passed。

**Tech Stack:** Python 3.12, dataclasses(frozen), pytest。无新依赖(schema 检查用最小自实现,YAGNI)。

**Files:**
- Create: `argos/contracts/__init__.py`, `types.py`, `runner.py`, `synthesize.py`
- Modify: `argos/core/verify_gate.py`(加 `verify_contract` 方法)
...
**Files:**
- Create: `argos/contracts/__init__.py`, `argos/contracts/types.py`
...
from argos.contracts.types import Check, Contract
...
Expected: FAIL with `ModuleNotFoundError: No module named 'argos.contracts'`

- [ ] **Step 3: Write the implementation**

```python
# argos/contracts/__init__.py
"""任务契约(Factory P1):完成的定义先于执行。"""
from argos.contracts.types import Check, Contract  # noqa: F401
```

```python
# argos/contracts/types.py
...
```bash
rtk git add argos/contracts tests/contracts
...
**Files:**
- Create: `argos/contracts/runner.py`
...
from argos.contracts.types import Check, Contract
from argos.contracts.runner import run_contract
...
```bash
rtk git add argos/contracts/runner.py tests/contracts/test_runner.py
...
**Files:**
- Create: `argos/contracts/synthesize.py`
...
from argos.contracts.synthesize import build_synthesis_prompt, parse_contract
...
```bash
rtk git add argos/contracts/synthesize.py tests/contracts/test_synthesize.py
...
**Files:**
- Modify: `argos/core/verify_gate.py`(`Verifier` 类内,`verify` 方法之后追加)
...
(若 `Verifier(workspace=...)` 构造签名不符,先 `grep -n "def __init__" argos/core/verify_gate.py`
按真实签名修测试夹具——改夹具,不改产品语义。)

- [ ] **Step 3: Write the implementation**

在 `argos/core/verify_gate.py` 的 `Verifier` 类内、`verify()` 方法之后追加:
...
(若 `Verifier` 的 workspace 属性名不同,先 `grep -n "workspace" argos/core/verify_gate.py`
对齐真实属性名。)
...
```bash
rtk git add argos/core/verify_gate.py tests/contracts/test_verifier_contract.py
...
**Files:**
- Modify: `argos/core/loop.py`(`run_verify_gate` 一带,先定位)
- Test: 在 `tests/test_loop.py` 风格上新增 `tests/contracts/test_loop_contract.py`

- [ ] **Step 1: 定位接缝**

Run: `grep -n "verifier.verify\|run_verify_gate" argos/core/loop.py`
...
```bash
rtk git add argos/core/loop.py tests/contracts/test_loop_contract.py
