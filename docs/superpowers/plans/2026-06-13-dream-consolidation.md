...
**背景：** `learning/reflection.py:46` 调 `capture_event("task_reflection", ...)`，但 `argos/memory/auto.py:635` 的 `_TYPE_MAP` 只认 5 种 kind，未知 kind 直接 `return None` —— 生产里失败反思从未落盘。

**Files:**
- Modify: `argos/memory/auto.py`（`_TYPE_MAP` / `_SCOPE_MAP` / `_DEFAULT_CONFIDENCE` 三表 + `capture_event` 的 value/key 分支）
...
```bash
rtk git add argos/memory/auto.py tests/test_memory_capture.py
...
**Files:**
- Create: `argos/learning/candidates.py`
...
- [ ] **Step 3: 实现 `argos/learning/candidates.py`**
...
```bash
rtk git add argos/learning/candidates.py tests/learning/test_candidates.py
...
**Files:**
- Modify: `argos/learning/hook.py`（`on_run_completed` 加 `workspace` kwarg；`_on_passed` 落盘分支）
- Modify: `argos/daemon/worker.py:654`（透传 workspace）
...
`argos/daemon/worker.py` `_maybe_run_learning_hook` 的调用处追加：
...
（`entry` 是 registry 的 run 条目；先 `grep -n "workspace" argos/daemon/registry.py` 确认字段名，若 entry 无 workspace 字段则从 `self._manager` 侧拿——以实际代码为准，测试只钉 hook 层。）
...
```bash
rtk git add argos/learning/hook.py argos/daemon/worker.py tests/learning/test_hook.py
...
**Files:**
- Modify: `argos/conductor/orders.py`（StandingOrder + to_dict/from_dict + __post_init__ 校验）
- Modify: `argos/conductor/proposals.py`（ProactiveSuggestion + propose 透传）
- Modify: `argos/protocol/events.py:285`（ProactiveSuggestionEvent）
- Modify: `argos/daemon/conductor_supervisor.py:120`（_emit_suggestion 透传）
...
def test_propose_carries_order_action():
    import time
    from argos.conductor.orders import StandingOrder
    from argos.conductor.proposals import propose
    o = StandingOrder(
...
```bash
rtk git add argos/conductor/orders.py argos/conductor/proposals.py argos/protocol/events.py argos/daemon/conductor_supervisor.py tests/conductor/test_orders.py tests/conductor/test_proposals.py tests/protocol/test_event_golden.py
...
**Files:**
- Create: `argos/learning/dream.py`（本任务只做 scan/cluster/synthesize 三个纯函数；管道编排在 Task 7）
- Test: `tests/learning/test_dream.py`

核心数据结构与铁律：

- `DreamUnit`：一个整合单元 = 1 个或多个同簇 `StoredCandidate`。
- 聚类相似度 v1 = goal+verify_cmd 的 token Jaccard（阈值 0.35，常量 `SIM_THRESHOLD`）；不引第三方库。**spec 的 embedding 路径有意延后**（见 Self-Review），接口留好：相似度函数独立可替换。
- `synthesize(unit, narrative=None)`：代码/verify 段逐字来自源候选（带 `source_run` 标注）；`narrative` 是**预先算好的叙述文本**（`str | None`）——模型调用发生在 DreamPipeline 层（Task 8，async 友好），`synthesize` 本身是纯函数（评审 B2 的 async 修正）；**`_strip_code_blocks()` 剥掉 narrative 里一切 fenced code block**；narrative 为 None/空 → 模板叙述兜底。
- `narrative_prompt(unit)`：构造叙述层提示词的纯函数（pipeline 调模型前用）。
...
- [ ] **Step 3: 实现 `argos/learning/dream.py`（本任务部分）**
...
def cluster_candidates(
    ...
    # 双车道上限(评审建议):多源簇与单例各自封顶,跨 run 综合(头牌价值)
    # 永远不会被单例挤掉,单例也不会饿死(积压的隔晚消化)
...
def _merged_name(unit: DreamUnit) -> str:
    from argos.learning.distiller import _slugify_goal
...
```bash
rtk git add argos/learning/dream.py tests/learning/test_dream.py
...
**Files:**
- Modify: `argos/learning/promotion_gate.py`（`promote` 加可选 `runner_b`）
- Modify: `argos/learning/dream.py`（追加 `build_eval_tasks` / `HintedRunner` / `promote_unit`）
...
def test_promote_runner_b_used_for_b_side(tmp_path):
    ...
    assert res.a_total == 2 and res.b_total == 2  # 计数器不许丢(评审 B2)
```
...
def test_build_eval_tasks_skips_missing_workspace(tmp_path):
    from argos.learning.dream import build_eval_tasks, cluster_candidates
    ws = tmp_path / "proj"
    ws.mkdir()
...
`promotion_gate.py`：**最小 diff，两处**（评审 B2 警告：现有循环体里的 `a_total += 1` / `b_total += 1` 计数器与外层 `try/except → runner_error` 守卫**一行都不许动**，只做以下两个插入）：

1. `promote` 签名加 `runner_b: Any = None`（放 `runner` 之后）；
2. 循环体内 B 侧调用前插一行选择器，并把 B 侧的 `runner.run` 改为 `rb.run`：

```python
    # 现有代码(promotion_gate.py:94-109)上下文,仅 ← 标注行是新增/改动:
    try:
        for task in tasks:
            try:
                ...
                a = None
            rb = runner_b if runner_b is not None else runner   # ← 新增
            try:
                b = rb.run(task, model_tier="default")          # ← runner → rb
            except Exception as e:  # noqa: BLE001
                b = None
            a_total += 1          # ← 原行保留
            b_total += 1          # ← 原行保留
            ...                   # _is_pass 判定原样
    except Exception as e:  # noqa: BLE001 — 外层守卫原样保留
        return PromotionResult(promoted=False, reason=f"runner_error:...")
```

（判定/落盘/builtin 硬拒/`a_total`/`b_total`/外层守卫一行不动。）
...
注意：`EvalTask` 若是 frozen dataclass 则 `dataclasses.replace` 可用；先确认（`grep -n "frozen" argos/eval/corpus.py`）。非 frozen 也兼容。
...
```bash
rtk git add argos/learning/promotion_gate.py argos/learning/dream.py tests/learning/test_promotion_gate.py tests/learning/test_dream.py
...
**Files:**
- Create: `argos/memory/consolidate.py`
- Modify: `argos/memory/auto.py`（`_decay_confidence` 提公开名 `decayed_confidence`，私有名留别名）
...
- [ ] **Step 3: 实现 `argos/memory/consolidate.py`**
...
```bash
rtk git add argos/memory/consolidate.py tests/test_memory_consolidate.py
...
**Files:**
- Modify: `argos/learning/dream.py`（追加 `DreamPipeline` / `DreamReport` / `has_material` / `scan_material`）
- Modify: `argos/protocol/events.py`（追加 `DreamProgressEvent` / `DreamReportEvent`）
...
def test_dream_progress_golden():
    from argos.protocol.events import DreamProgressEvent
...
def test_dream_report_golden_and_roundtrip():
    from argos.protocol.events import DreamReportEvent
    ev = DreamReportEvent(
...
from argos.learning.candidates import save_candidate, list_unconsumed
...
**注册表三处缺一不可（评审 B4：漏注册 → `deserialize_event` 直接 raise）：**

1. `EventKind` Literal（events.py:28-48）追加 `"dream_progress"`、`"dream_report"`；
2. `_KIND_TO_CLASS` dict（events.py:399-416）追加两个映射，仿 `ProactiveSuggestionEvent`（:413）的写法；
3. golden 测试里跑一次 `_round()`（上面的测试已含），确保 serialize → deserialize 闭环。
...
class DreamPipeline:
    ...
    def __init__(self, *, candidates_root: Path, skills_root: Path,
        ...
        # 锁懒初始化(评审 B3):在 __init__ 创建会绑到构造时的事件循环,
        # 测试 asyncio.run 换循环后单飞锁会静默失效
    ...
    async def run(self) -> DreamReport | None:
        ...
        if self._lock is None:
            self._lock = asyncio.Lock()   # 懒初始化:绑当前运行中的事件循环
        ...
        async with self._lock:
            ...
            for unit in units:
                try:
                    # 叙述层:模型调用在这里(async 友好,评审 B2);失败 → None → 模板兜底
                    narrative = None
    ...
    def _write_report_line(self, *, units: int, promoted: int, rejected: int,
        ...
        try:
            ...
            from argos.jsonl_log import append_line  # 实名 append_line(jsonl_log.py:31,评审 B1 已核)
            append_line(p, {
...
（`jsonl_log` 的实际函数名先 `grep -n "def " argos/jsonl_log.py` 确认；对不上就用 `open("a")` 直写同语义。）
...
```bash
rtk git add argos/learning/dream.py argos/protocol/events.py tests/learning/test_dream_pipeline.py tests/protocol/test_event_golden.py
...
**Files:**
- Modify: `argos/daemon/conductor_supervisor.py`（builtin order 注册 + 材料门过滤）
- Modify: `argos/daemon/server.py`（confirm 按 action 路由 + `POST /dream/run` + `GET /dream/report`）
...
```python
    def _build_narrate(self):
        """叙述层模型调用(async callable;无 components/无 model → None,模板兜底)。

        DreamPipeline 在 daemon 事件循环内 await 它(评审 B2:绝不
        get_event_loop / run_coroutine_threadsafe —— 3.12 里前者 raise,
        后者同循环死锁)。超时由 asyncio.wait_for 钉死,模型 hang 不拖管道。
        """
...
```
   ...
   （`DreamPipeline.run()` 内部用 `inspect.isawaitable` 分流（Task 8 已实现），所以这里返回 async callable、测试里传 sync fake 都兼容。）
...
```bash
rtk git add argos/daemon/conductor_supervisor.py argos/daemon/server.py tests/conductor/test_daemon_wiring.py
...
**Files:**
- Modify: `argos/tui/app.py`（仿 `:642` 的 `elif cmd.name == "orders":` 模式加 `dream`；slash 注册表同步——先 grep 该文件 slash 命令注册结构）
- Create: `argos/cli/dream.py`
- Modify: `argos/__main__.py`（`sub.add_parser("dream", ...)` + dispatch，仿 `eval` 子命令接法）
...
```bash
rtk git add argos/tui/app.py argos/cli/dream.py argos/__main__.py tests/
...
- **与 spec 的有意偏离（共 3 处）**：① `kind` 字段实现为 `action`（避开 `StandingOrder.kind` 占用，Task 4）；② **embedding 聚类延后**——spec §2 承诺 "有 embedder 用 embedding"，v1 只实现 token Jaccard（夜间路径省 embed 成本/延迟），相似度函数已独立可替换，embedding 接入留 follow-up；③ 记忆整理打分**不含 use_count 回血项**——复用 auto.py 既有 `_decay_confidence` 单一公式（回血在召回时通过 `last_used_at` 更新体现），spec 的 "+0.02 use-count" 表述以现有实现为准。
- **类型一致性**：`StoredCandidate`（T2 定义，T5/T6/T8 使用，含 `self_verified`）、`DreamUnit`（T5 定义，T6/T8 使用）、`promote(runner_b=)`（T6 定义，T8 调用）、`narrative_prompt`/`synthesize(narrative=)`（T5 定义，T8 调用）、`has_material`（T8 定义，T9 调用）已对齐。
- **已知风险**（实现者注意）：① `_slugify_goal` 私有复用（T5 内注明备选）；② `entry.workspace` 字段名（T3 内注明先 grep registry；eng 评审已核 registry.py:40 有 `workspace: str`）。
- **遗留 TODO（不阻塞）**：`~/.argos/dreams/` 报告无保留期轮转（每晚一文件），follow-up 加 "保留最近 90 天" 修剪。

## 评审记录（第 1 轮，2026-06-13）

| 评审 | 模型 | 裁定 | blocking | 处置 |
|---|---|---|---|---|
| plan-ceo-review | opus | revise | 2（E4 候选来源记录缺失；promote 补丁丢计数器） | 均已回修：T2/T3 加 `self_verified` 落盘+材料层双保险+spy 测试；T6 改最小 diff + 计数器断言 |
| plan-eng-review | sonnet | revise | 4（append_jsonl 不存在；get_event_loop 3.12 炸；Lock 跨循环失效；dream 事件未注册） | 均已回修：T8 改 `append_line`+懒锁+注册表三步；T9 narrate 改 async callable + wait_for |
