...
- Modify: `argos_agent/memory/auto.py`（`_decay_confidence` 提公开名 `decayed_confidence`，私有名留别名）
...
class DreamPipeline:
    ...
    def _write_report_line(self, *, units: int, promoted: int, rejected: int,
        ...
        try:
            ...
            from argos_agent.jsonl_log import append_line  # 实名 append_line(jsonl_log.py:31,评审 B1 已核)
            append_line(p, {
