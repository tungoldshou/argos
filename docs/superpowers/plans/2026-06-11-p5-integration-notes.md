...
| 文件 | 职责 |
|---|---|
| `argos_agent/conductor/__init__.py` | 公开 API |
| `argos_agent/conductor/orders.py` | `StandingOrder` + `OrderStore` JSONL 持久化 |
| `argos_agent/conductor/cronlite.py` | 零依赖 cron-lite（`next_due`）|
| `argos_agent/conductor/triggers.py` | `FileTriggerWatcher` + `FileTriggerFact` |
| `argos_agent/conductor/proposals.py` | `ProactiveSuggestion` + `propose()` |
| `argos_agent/conductor/engine.py` | `ConductorEngine`（`tick()` 驱动）|
...
**接入文件**：`argos_agent/daemon/worker.py`（或 daemon 主循环）
...
**接入文件**：`argos_agent/daemon/server.py`（ACP HTTP 路由）
