...
>
> **本计划不动**:`argos_agent/skills.py`(LLM 提示召回用,沿用),`argos_agent/skills_runtime/`
> (3 个 builtin skill + 编排,沿用),`argos_agent/approval.py`(审批逻辑,沿用)。
>
> **新代码全部在**:`argos_agent/skills_curator/`(7 个新模块)+ `argos_agent/cli/skills.py`
...
### 1.1 目标
- 新文件 `argos_agent/skills_curator/__init__.py` + `index.py`
...
### 1.2 实现(节选)
```python
# argos_agent/skills_curator/index.py
...
### 2.1 目标
- 新文件 `argos_agent/skills_curator/capabilities.py`
...
### 2.2 实现(节选)
```python
# argos_agent/skills_curator/capabilities.py
...
from argos_agent.skills_curator.index import VALID_CAPABILITIES, BUILTIN_NAMES
...
### 2.3 CLI `cmd_list` 实现
```python
# argos_agent/cli/skills.py (加)
def cmd_list(args: argparse.Namespace) -> int:
    from argos_agent.skills_curator.capabilities import list_installed
    from argos_agent.skills_curator.index import load_cache, cache_age_days
...
### 3.1 目标
- 新文件 `argos_agent/skills_curator/install.py`
...
### 3.2 实现(节选)
```python
# argos_agent/skills_curator/install.py
...
from argos_agent.skills_curator.capabilities import (
...
def cmd_install(args: argparse.Namespace) -> int:
    ...
    name = args.name
    # 网络 skill 二次确认
    from argos_agent.skills_curator.index import load_cache
    cache = load_cache()
    if cache:
        entry = cache.find(name)
...
### 4.1 目标
- 新文件 `argos_agent/skills_curator/remove.py`
...
### 4.2 实现(节选)
```python
# argos_agent/skills_curator/remove.py
...
from argos_agent.skills_curator.index import BUILTIN_NAMES, _skills_root
from argos_agent.skills_curator.install import backup_to_trash, InstallError
...
def cmd_remove(args: argparse.Namespace) -> int:
    from argos_agent.skills_curator.remove import remove, InstallError
    try:
        r = remove(args.name)
    except InstallError as e:
        print(f"[skills] error: {e}", file=__import__("sys").stderr)
...
### 5.1 目标
- 新文件 `argos_agent/skills_curator/smoke.py`
...
### 5.2 实现(节选)
```python
# argos_agent/skills_curator/smoke.py
...
async def _skills_cmd(self, log, arg: str) -> None:
    from argos_agent.skills_curator.capabilities import list_installed
    from argos_agent.skills_curator.index import load_cache, cache_age_days
    from argos_agent.skills_curator.recommend import recommend, SessionActivity
    if not arg.strip():
...
async def _skills_list(self, log) -> None:
    ...
    # 推荐(spec §8)
    try:
        from argos_agent.skills_curator.recommend import build_activity_from_session, recommend
...
### 7.1 目标
- 新文件 `argos_agent/skills_curator/recommend.py`
...
### 7.2 实现(节选)
```python
# argos_agent/skills_curator/recommend.py
