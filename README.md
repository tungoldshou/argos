...
一个独立的终端(TUI)编码超级智能体:**单个 Python 进程**,基于 Textual。它通过
...
> 不只是编码智能体:Argos 的目标是 **Claude Code + 通用 agent** 的合体 —— 在写代码之外,
> 还能联网检索、按需调技能(Skills)、对结构化任务注入"必检约定"契约,逐步补齐计算机控制
> 与 MCP 等能力。每一项能力都遵循同一条底线:**能验证才说完成,不行就如实说,绝不假装。**
...
- **`argos_agent/tools/`** — 注入沙箱命名空间的工具:文件读写编辑搜索、白名单 shell、
  联网检索/正文提取、`propose_verify`(独立验证门)、`update_plan`(真 TODO 拆解)。
