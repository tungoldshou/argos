...
# 每行: (TrustLevel, 行标签(short), hint 列文本)
...
_DIAL_ROWS: list[tuple[TrustLevel, str, str]] = [
    ...
    # L4: hint 列包含 ⏻($fail) 红灯 + 余下文字($ink-faint)
    (TrustLevel.L4_AUTONOMOUS,        "全自治",               "⏻ 红灯 · HARD RULES 仍拦"),
]

# L4 hint 分隔:⏻ 红灯部分 + 余下部分
_L4_HINT_RED  = "⏻ 红灯"         # 使用 $fail
_L4_HINT_REST = " · HARD RULES 仍拦"  # 使用 $ink-faint
...
class TrustDial(Static):
    ...
    def _compose_text(self) -> Text:
        ...
        # ── 第 2–6 行:五行拨盘 ────────────────────────────────────────────
        for lvl, label, hint in _DIAL_ROWS:
            ...
            if is_current:
                ...
                if lvl is TrustLevel.L4_AUTONOMOUS:
                    t.append(_L4_HINT_RED, style=_COL_FAIL)
                    t.append(_L4_HINT_REST, style=_COL_INK)
                else:
                    t.append(hint, style=_COL_INK)
            else:
                ...
                if lvl is TrustLevel.L4_AUTONOMOUS:
                    t.append(_L4_HINT_RED, style=_COL_FAIL)
                    t.append(_L4_HINT_REST, style=_COL_INK_FAINT)
                else:
                    t.append(hint, style=_COL_INK_FAINT)

            t.append("\n")

        # ── 铁律行:HARD RULES 永不降级(任何档位下均渲染) ──────────────────
        # 规格 §10 line 293: "三处 $fail"
        t.append("HARD RULES 永不降级:", style=_COL_INK_DIM)
        t.append("危险 shell", style=_COL_FAIL)
        t.append(" · ", style=_COL_INK_DIM)
        t.append("系统路径", style=_COL_FAIL)
        t.append(" · ", style=_COL_INK_DIM)
        t.append("secret 检测", style=_COL_FAIL)
        t.append("\n")

        # ── Footer(可选 provenance,$ink-faint)──────────────────────────────
        t.append("升档必带警示 · 绝不静默自动升", style=_COL_INK_FAINT)
        t.append("  ", style=_COL_INK_FAINT)
        t.append("permissions/trust_dial", style=_COL_INK_FAINT)
