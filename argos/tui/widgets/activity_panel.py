...
from argos.tui.widgets._fmt import fmt_cost, fmt_token_flow, fmt_tokens
...
class ActivityPanel(Vertical):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield _Section(t_("widget.section_cost"), f"{fmt_token_flow(0, 0)}  {fmt_cost(0.0)}\n" + t_("widget.cache_hit_line", cache_read=0, elapsed_s=0.0))
    ...
    def on_cost(self, *, tokens_in: int, tokens_out: int, cost_usd: float | None,
        ...
        cost = fmt_cost(cost_usd)   # 未知 → 'N/A'(不再 '$(N/A)' shell 样);共享 _fmt
        ...
        t = Text()
        t.append(f"{fmt_token_flow(tokens_in, tokens_out)}{tier_tag}  {cost}\n")
        if self._cache_seen:
